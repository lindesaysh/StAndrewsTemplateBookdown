...
In this Chapter we are going to introduce *extra* features/environments exclusive to the St Andrews template. 
...
`task` and `solution` boxes can be given different names using the `title` option. For example:
...
By default, in `_setup.Rmd`, `renderSol=TRUE`. If one of your chapters is a tutorial or practical, it can become tedious having to set `renderSol` to `FALSE` for every question. 
Instead, you can override this default at the beginning of each chapter, so you can turn on/off the solutions, as follows:

````

``{r, include=F}
opts_chunk$set(renderSol=TRUE)
``
